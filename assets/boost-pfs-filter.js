...
(function() {
  ...
  ProductGridItem.prototype.compileTemplate = function(data, index) {
    ...
    for (var i = 0; i < data.tags.length; i++) {
      var tag = data.tags[i].toLowerCase();
      if (tag.indexOf('length_') > -1) {
        length = tag.replace('length_', '');
        lengthCount++;
      }
      if (tag.toLowerCase().indexOf('width_') > -1) {
        width = tag.replace('width_', '');
        widthCount++;
      }
    }
